...
class ReportParser:
    ...
    def _generate_kv(self, is_kv_persisted=False, output_dir="output"):
        """
        ...
        if is_kv_persisted:
            kv_output_path = os.path.join(output_dir, "kv", f"{self.file_name}_kv.csv")
    ...
    def process_file(self, output_dir, is_kv_persisted=False):
        ...
        kv = self._generate_kv(is_kv_persisted=is_kv_persisted, output_dir=output_dir)
